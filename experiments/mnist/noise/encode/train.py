...
Options:
  ...
  -o, --out=<output_dir>  Output directory.
  -c, --config=<config>   Config file name [default: config].
  --noise-level=<frac>    Noise level.
"""
...
def train_step(x: Any, encoder: nn.Module, decoder: nn.Module, kernel: Any, state: TrainState,
  ...
  def loss_fn(params):
    ...
    equiv_rel = (
        jnp.linalg.norm(Tz0_p - Tz0_b) / (jnp.linalg.norm(Tz0_b) + 1e-8) +
        jnp.linalg.norm( z0_p -  z0_b) / (jnp.linalg.norm( z0_b) + 1e-8)
    )

    if not train:
        jax.debug.callback(
            lambda v, s: wandb.run.log({"eval_equiv_rel": float(v)}, step=int(s)),
            equiv_rel, step
        )
...
def viz_results( cfg, z, batch, x_out, tauOmega, Omega, mode="train", std_factor=2.5, image_dir=None):
  ...
  ###
  # Extra visualizations for entire batch (original | noisy | recon_original | recon_noisy)
  x_panels = []

  batchN = np.reshape(batch, (-1, 2, batch.shape[1], batch.shape[2], batch.shape[3]))

  for i in range(batchN.shape[0]):
    original   = np.asarray(255.0 * np.clip(batchN[i, 0, ..., 0], 0, 1)).astype(np.uint8)
    noisy   = np.asarray(255.0 * np.clip(batchN[i, 1, ..., 0], 0, 1)).astype(np.uint8)
    recon_orig = np.asarray(255.0 * np.clip(x_out[i, 2, ..., 0], 0, 1)).astype(np.uint8)
    recon_noisy = np.asarray(255.0 * np.clip(x_out[i, 3, ..., 0], 0, 1)).astype(np.uint8)

    panel = np.concatenate([original, noisy, recon_orig, recon_noisy], axis=1)
    x_panels.append(wandb.Image(panel, caption="original | noisy | recon_orig | recon_noisy [{}]".format(i)))

  wandb_ims["{}_batch_reconstructions".format(mode)] = x_panels
  ###

  return wandb_ims 


def compute_additional_metrics(tauOmega, z_batch, Omega, step, mode="eval"):
    """Compute latent equivariance error and block structure degradation, and log to wandb."""

    # Squeeze dimensions
    if tauOmega.ndim == 4:  # (devices, batch, spatial_dim, spatial_dim)
        tauOmega = tauOmega[0, 0, ...]
    elif tauOmega.ndim == 3:  # (batch, spatial_dim, spatial_dim)
        tauOmega = tauOmega[0, ...]

    if z_batch.ndim == 5:  # (devices, batch, H, W, latent_dim)
        z_batch = z_batch[0, 0, ...]
    elif z_batch.ndim == 4:
        z_batch = z_batch[0, ...]

    if isinstance(Omega, (tuple, list)) and len(Omega) == 3:
        Omega = (Omega[0][0], Omega[1][0], Omega[2][0])

    # Shapes
    H, W, latent_dim = z_batch.shape
    spatial_size = H * W

    latent_flat = jnp.reshape(z_batch, (spatial_size, latent_dim))  # (400, 32)

    Phi = Omega[0]  # (400, 32)

    # Project into eigenbasis
    latent_projected = jnp.einsum('ij,jd->id', Phi.T, latent_flat)  # (32, 32)

    # Now split across latent dimensions, not spatial points
    Epsi = latent_projected[:, :latent_projected.shape[1]//2]  # (32, 16)
    Epsi_T = latent_projected[:, latent_projected.shape[1]//2:]  # (32, 16)

    def apply_tau(latent, tauOmega):
        return jnp.einsum('ij,jd->id', tauOmega, latent)

    Epsi_mapped = apply_tau(Epsi, tauOmega)

    def compute_equivariance_error(Epsi_mapped, Epsi_T):
        numer = jnp.linalg.norm(Epsi_mapped - Epsi_T)
        denom = jnp.linalg.norm(Epsi_T) + 1e-8
        return numer / denom

    def compute_off_diagonal_energy(tauOmega):
        spatial_dim = tauOmega.shape[0]
        identity = jnp.eye(spatial_dim)
        off_diag_mask = 1.0 - identity
        off_diag_energy = jnp.sum((tauOmega * off_diag_mask)**2)
        total_energy = jnp.sum(tauOmega**2)
        return off_diag_energy / (total_energy + 1e-8)

    equiv_error = compute_equivariance_error(Epsi_mapped, Epsi_T)
    block_energy = compute_off_diagonal_energy(tauOmega)

    wandb.log({
        f"{mode}/equivariance_error": equiv_error,
        f"{mode}/block_structure_energy": block_energy
    }, step=step)

    tauOmega_vis = tauOmega / (jnp.max(jnp.abs(tauOmega)) + 1e-8)
    tauOmega_vis = (tauOmega_vis + 1.0) / 2.0
    tauOmega_vis = np.asarray(tauOmega_vis * 255.0, dtype=np.uint8)

    wandb.log({
        f"{mode}/tauOmega": [wandb.Image(tauOmega_vis, caption=f"{mode} τΩ at step {step}")]
    }, step=step)
...
def train_and_evaluate( cfg: Any, output_dir: str ):
  ...
  exp_name     =  "niso_{}_kopd_{}_ld_{}d_mnist_noise".format(cfg.KERNEL_OP_DIM, cfg.LATENT_DIM, len(cfg.CONV_ENC_CHANNELS)-1, cfg.DATASET_NAME)  
  ...
  wandb.log({"dataset": "MNIST", "transformation": "gaussian_noise", "noise_level": noise_level})
  ...
  xform = xforms.Noise(noise_level=noise_level)
  ...
  for step in tqdm( range(initial_step, stop_at) ):
    ...
    if step % cfg.EVAL_EVERY == 0 or is_last_step:
      ...
      for j in range( cfg.NUM_EVAL_STEPS ):
        ...
        metrics_dict = flax_utils.unreplicate(emetrics_update).compute()
        print(f"Eval Step {step}: "
              f"eval_loss = {metrics_dict['eval_loss']:.6f}, "
              f"eval_recon_loss = {metrics_dict['eval_recon_loss']:.6f}, "
              f"eval_equiv_loss = {metrics_dict['eval_equiv_loss']:.6f}, "
              f"eval_mult_loss = {metrics_dict['eval_mult_loss']:.6f}")

        compute_additional_metrics(tauOmega, z, Omega, step=step, mode="eval")
...
if __name__ == '__main__':
  ...
  noise_level = float(arguments['--noise-level'])
