...
Options:
  ...
  -o, --out=<output_dir>  Output directory [default: {root_path}/experiments/hmnist_occlude/weights/].
  -c, --config=<config>   Config file name [default: config].
  --box-frac=<frac>       Box size as fraction of image size.
"""
...
def viz_results( cfg, z, batch, x_out, tauOmega, Omega, mode="train", std_factor=2.5, image_dir=None):
  ...
  ###
  # Extra visualizations for entire batch (original | occluded | recon_original | recon_occluded)
  x_panels = []
  ...
  for i in range(batchN.shape[0]):
    ...
    occluded   = np.asarray(255.0 * np.clip(batchN[i, 1, ..., 0], 0, 1)).astype(np.uint8)
    recon_orig = np.asarray(255.0 * np.clip(x_out[i, 2, ..., 0], 0, 1)).astype(np.uint8)
    recon_occl = np.asarray(255.0 * np.clip(x_out[i, 3, ..., 0], 0, 1)).astype(np.uint8)

    panel = np.concatenate([original, occluded, recon_orig, recon_occl], axis=1)
    x_panels.append(wandb.Image(panel, caption="original | occluded | recon_orig | recon_occl [{}]".format(i)))
...
def train_and_evaluate( cfg: Any, output_dir: str ):
  ...
  exp_name     =  "niso_{}_kopd_{}_ld_{}d_mnist_occlude".format(cfg.KERNEL_OP_DIM, cfg.LATENT_DIM, len(cfg.CONV_ENC_CHANNELS)-1, cfg.DATASET_NAME)  
  ...
  wandb.log({"dataset": "MNIST", "transformation": "occlusion_rotation", "box_frac": box_frac})
  ...
  xform = xforms.OcclusionRotation(box_frac=box_frac)
...
if __name__ == '__main__':
  ...
  box_frac = float(arguments['--box-frac'])
