...
def log_predictions_to_wandb(images, labels, logits, mode="eval", max_samples=16):
  """Logs randomly sampled images with predicted and true labels to W&B."""
  images = np.asarray(images)
  labels = np.asarray(labels)
  preds = np.argmax(np.asarray(logits), axis=-1)

  # Unnormalize if needed and convert to uint8
  ...
  wandb_images = []
  for i in indices:
    img = images[i, ..., 0] if images.shape[-1] == 1 else images[i]
    caption = f"Pred: {preds[i]} | True: {labels[i]}"
    wandb_images.append(wandb.Image(img, caption=caption))

  wandb.log({f"{mode}/predictions": wandb_images})
...
def get_logits(params, x, model, encoder, kernel):
  z = encoder.apply({"params": params["encoder"]}, x)
  z = jnp.reshape(z, (z.shape[0], -1, z.shape[-1]))
  z = jax.lax.stop_gradient(z)
...
def train_and_evaluate( cfg: Any, input_dir: str, output_dir: str, load_weight_dir: str):
  ...
  ========== Setup W&B =============
  '''
  exp_name     =  "niso_{}_kop_{}_ld_{}d_mnist_elastic_deformation_pred".format( cfg.KERNEL_OP_DIM,
                                                            cfg.LATENT_DIM,
    ...
    if step % cfg.EVAL_EVERY == 0 or is_last_step:
      ...
      for batch in tqdm(test_data):
        ...
        if eval_metrics is None:
          logits = get_logits(flax_utils.unreplicate(state).params, batchIm, model, encoder, kernel)
          log_predictions_to_wandb(batchIm, labels, logits, mode="eval", max_samples=16)
