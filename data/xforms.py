...
class Occlusion:
  ...
  def apply(self, img, key):
    ...
    # Force input to be float32 in [0, 1] range
    img = jnp.clip(img, 0.0, 1.0).astype(jnp.float32)

    # Use fixed-size patch (avoid dynamic shapes)
    box_h = int(H * self.max_frac)
    box_w = int(W * self.max_frac)

    # Random top-left corner
    top = jax.random.randint(key1, (), 0, H - box_h + 1)
    left = jax.random.randint(key2, (), 0, W - box_w + 1)

    # Start with all-ones mask
    mask = jnp.ones((H, W, 1), dtype=jnp.float32)

    # Create all-zero patch
    zero_patch = jnp.zeros((box_h, box_w, 1), dtype=jnp.float32)

    # Update mask to zero out selected region
    mask = lax.dynamic_update_slice(mask, zero_patch, (top, left, 0))

    # Apply mask
    occluded_img = img * mask
    return occluded_img

  def draw_and_apply(self, x, key):
    keys = jax.random.split(key, x.shape[0])
    occluded = jax.vmap(lambda img, k: self.apply(img, k))(x, keys)

    return jnp.reshape(
        jnp.concatenate((x[:, None, ...], occluded[:, None, ...]), axis=1),
        (-1, x.shape[1], x.shape[2], x.shape[3])
    )


class OcclusionRotation:
  def __init__(self, box_frac=0.3):
    self.box_frac = box_frac

  def _make_rotated_square_mask(self, H, W, box_size, angle_deg, cx, cy):
  ...
  def apply(self, img, key):
    ...
    box_size = int(H * self.box_frac)
    cx = jax.random.uniform(key1, (), minval=box_size // 2, maxval=H - box_size // 2)
    cy = jax.random.uniform(key2, (), minval=box_size // 2, maxval=W - box_size // 2)
    angle = jax.random.uniform(key3, (), minval=0.0, maxval=180.0)

    mask = 1.0 - self._make_rotated_square_mask(H, W, box_size, angle, cx, cy)
    return img * mask
...
class Noise:
    ...
    def apply(self, img, key):
        ...
        # Add Gaussian noise
        noise = random.normal(key, img.shape) * self.noise_level
        noisy_img = jnp.clip(img + noise, 0.0, 1.0)
        return noisy_img

    def draw_and_apply(self, x, key):
        keys = jax.random.split(key, x.shape[0])
        noisy = jax.vmap(lambda img, k: self.apply(img, k))(x, keys)

        # Stack original + noisy for visualization
        return jnp.reshape(
            jnp.concatenate((x[:, None, ...], noisy[:, None, ...]), axis=1),
            (-1, x.shape[1], x.shape[2], x.shape[3])
        )
...
class ElasticDeform:
  ...
  def apply(self, img, key):
    ...
    dx = self._gaussian_blur(dx, sigma=self.sigma) * self.alpha
    dy = self._gaussian_blur(dy, sigma=self.sigma) * self.alpha

    x, y = jnp.meshgrid(jnp.arange(W), jnp.arange(H))
    indices_x = jnp.clip(x + dx, 0, W - 1)
    indices_y = jnp.clip(y + dy, 0, H - 1)

    return self._bilinear_sample(img, indices_x, indices_y)

  def draw_and_apply(self, x, key):
    keys = random.split(key, x.shape[0])
    deformed = jax.vmap(lambda img, k: self.apply(img, k))(x, keys)

    return jnp.reshape(
      jnp.concatenate((x[:, None, ...], deformed[:, None, ...]), axis=1),
      (-1, x.shape[1], x.shape[2], x.shape[3])
    )
